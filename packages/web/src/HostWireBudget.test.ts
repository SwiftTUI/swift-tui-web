...
import { fitsWireGrid, HOST_WIRE_MAX_RECORD_BYTES } from "./HostWireBudget.ts";
...
for (const entry of fixture.cases) {
  test(`shared wire boundary ${entry.kind} ${entry.value ?? entry.width}`, () => {
    ...
    switch (entry.kind) {
      case "recordBytes":
        frame.future = padded(
          value - encoder.encode(wire({ ...frame, future: "" })).length + 1,
        );
        break;
      case "width":
        frame.width = value;
      ...
      case "grid":
        ...
        break;
      case "cellTextBytes":
        frame.rows = [[[0, padded(value), 1, 0]], []];
        break;
      case "styleBytes":
        frame.styles = [{ future: padded(value - 22) }];
        break;
      case "styles":
        frame.styles = Array(value).fill(null);
        break;
      case "images":
        frame.images = Array.from({ length: value }, () => ({
          id: "i",
          format: "future",
          bounds: [0, 0, 0, 0],
        ...
        break;
      case "rows":
        frame.rows = Array.from({ length: value }, () => []);
        break;
      case "metadataEntries":
        frame.linkTargets = Array(value).fill("");
        break;
      case "imageDimension":
      case "imagePixels":
      case "imageIdBytes":
        frame.images = [
          {
            id: entry.kind === "imageIdBytes" ? padded(value) : "i",
            format: "future",
            bounds: [0, 0, 0, 0],
            visibleBounds: [0, 0, 0, 0],
            scalingMode: "stretch",
            pixelSize:
              entry.kind === "imagePixels"
                ? [entry.width, entry.height]
                : entry.kind === "imageDimension"
                  ? [value, 1]
                  : [1, 1],
          },
        ];
...
test("byte-at-a-time framing preserves UTF-8 and exact record boundary decisions", () => {
  ...
  const decoder = new WebHostOutputDecoder();
  const result = [...encoder.encode(record)].flatMap((byte) =>
    decoder.feed(Uint8Array.of(byte)),
  );
  expect(result).toEqual(
    new WebHostOutputDecoder().feed(encoder.encode(record)),
  );
  const oversized = encoder.encode(
    prefix + "x".repeat(HOST_WIRE_MAX_RECORD_BYTES),
  );
  const fragmented = new WebHostOutputDecoder();
  expect(
    fragmented.feed(oversized.subarray(0, HOST_WIRE_MAX_RECORD_BYTES)),
  ).toEqual([]);
  expect(
    fragmented
      .feed(oversized.subarray(HOST_WIRE_MAX_RECORD_BYTES))
      .map((record) => record.type),
  ).toEqual(["surfaceDropped"]);
  expect(fragmented.flush()).toEqual([]);
  expect(fragmented.feed(encoder.encode(wire(base())))[0]?.type).toBe(
    "surface",
  );
});
...
test("style append and invalid deltas cannot grow or replace the retained baseline", () => {
  const decoder = new WebHostOutputDecoder();
  decoder.feed(
    encoder.encode(wire({ ...base(), styles: Array(1024).fill(null) })),
  );
  const delta = {
    version: 3,
    encoding: "delta",
  ...
  expect(decoder.feed(encoder.encode(wire(delta)))[0]?.type).toBe(
    "surfaceDropped",
  );
  // Inspect the same baseline using an empty valid append (no new table allocation).
  expect(
    decoder.feed(encoder.encode(wire({ ...delta, styles: [] })))[0]?.type,
  ).toBe("surface");
  for (const rows of [
    [[2, []]],
    [
...
test("a dense 256 by 256 grid stays supported", () => {
  ...
  const frame = {
    ...
    rows: Array.from({ length: 256 }, () =>
      Array.from({ length: 256 }, (_, x) => [x, "a", 1, 0]),
    ),
  };
  expect(
    new WebHostOutputDecoder().feed(encoder.encode(wire(frame)))[0]?.type,
  ).toBe("surface");
});
