...
import {
  ...
  MAX_RASTER_PIXELS,
} from "./RasterAllocationBudget.ts";

test("backing-store allocation preserves ordinary DPR and bounds large grids before sizing canvas", () => {
  expect(boundedCanvasSize(800, 600, 2)).toEqual({
    width: 1600,
    height: 1200,
    scale: 2,
  });
  expect(boundedCanvasSize(81, 41, 1.25)).toEqual({
    width: 102,
    height: 52,
    scale: 1.25,
  });
  ...
  expect(boundedCanvasSize(Infinity, 100, 2)).toEqual({
    width: 1,
    height: 1,
    scale: 1,
  });
