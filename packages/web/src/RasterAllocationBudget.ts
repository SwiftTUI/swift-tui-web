...
export function boundedCanvasSize(cssWidth: number, cssHeight: number, scale: number): {
  ...
  if (requestedWidth <= MAX_RASTER_DIMENSION && requestedHeight <= MAX_RASTER_DIMENSION
    && requestedWidth * requestedHeight <= MAX_RASTER_PIXELS) {
    return { width: requestedWidth, height: requestedHeight, scale };
  }
  const boundedScale = Math.min(scale, MAX_RASTER_DIMENSION / cssWidth,
    MAX_RASTER_DIMENSION / cssHeight, Math.sqrt(MAX_RASTER_PIXELS / cssWidth / cssHeight));
  return { width: Math.max(1, Math.floor(cssWidth * boundedScale)),
    height: Math.max(1, Math.floor(cssHeight * boundedScale)), scale: boundedScale };
}
