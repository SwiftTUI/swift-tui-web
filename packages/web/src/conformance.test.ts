...
import { conformanceSHA256 } from "./ConformanceFixtureBytes.ts";
import {
...
const fixtureDirectory = join(import.meta.dir, "../../../Fixtures/Transport");
...
describe("host-wire conformance corpus", () => {
  test("loads the full ten-body census including host-only fixtures", async () => {
    ...
    expect(
      corpus.manifest.fixtures
        .filter((entry) => entry.requiresStage === "s3d")
  ...
  for (const runner of WEB_CONFORMANCE_RUNNERS) {
    test(`${runner} executes every applicable active fixture`, async () => {
      const corpus = await loadConformanceCorpus(fixtureDirectory);
      const expected = corpus.manifest.fixtures.filter(
        (entry) =>
  ...
  test("Canvas reports unique eligible current image attachments", async () => {
    ...
    const fixture = syntheticFixture(
      [
        ...
        {
          expect: {
            rows: [{ row: 0, cells: [{ column: 0, text: " ", span: 1 }] }],
...
describe("host-wire conformance integrity and schema teeth", () => {
  test("rejects manifest hash, body hash, and hashed-body corruption independently", async () => {
    ...
    const header = JSON.parse(
      originalText.slice(0, originalText.indexOf("\n")),
    ) as {
    ...
    for (const corruptedText of [
      ...
      await expect(
        loadConformanceCorpus(fixtureDirectory, {
  ...
  test("binds adjacent drops backwards and refuses to cross a barrier", () => {
    ...
    expect(() => syntheticFixture([emitA, expectation, { drop: 1 }])).toThrow(
      /barrier/,
    );
  });
  ...
  test("derives inactive channel record metadata from each raw occurrence", () => {
    ...
    for (const record of invalidMetadata) {
      ...
      ).toThrow(/metadata does not match raw record/);
    }
    ...
    ).toThrow(/both delivered and suppressed/);
  });
});
...
describe("host-wire conformance observable-axis teeth", () => {
  ...
  const corruptions: Array<
    [string, (value: WebConformanceObservation) => void]
  > = [
    ...
    [
      "request scope",
      (value) => (value.resyncRequests[0] = { scope: "keyframe" }),
    ],
    [
      "request IDs",
      (value) => {
...
async function readCorpusBytes(): Promise<{
  ...
  for (const name of (await readdir(fixtureDirectory)).sort()) {
    if (/^conformance-.*\.jsonl$/.test(name)) {
      fixtures.set(
        name,
        new Uint8Array(await readFile(join(fixtureDirectory, name))),
      );
    }
  }
...
function flipLeadingHexDigit(hash: string): string {
...
function concatBytes(lhs: Uint8Array, rhs: Uint8Array): Uint8Array {
...
function channelExpectation(overrides: Record<string, unknown> = {}): unknown {
  return {
    expect: {
      deliveredRecords: [],
...
function surfaceRecord(payload: Record<string, unknown>): string {
