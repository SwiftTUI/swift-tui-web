...
export interface GlyphGeometrySink {
...
export function canRenderGeometricGlyph(text: string): boolean {
...
export function emitGlyphGeometry(
  context: GlyphGeometrySink,
  text: string,
  rect: Rect,
): boolean {
  const codePoint = singleCodePoint(text);
...
function drawArc(context: GlyphGeometrySink, rect: Rect, corner: Corner): void {
  const metrics = strokeMetrics(rect);
  const cx = rect.x + rect.width / 2;
  const cy = rect.y + rect.height / 2;
  const maxX = rect.x + rect.width;
