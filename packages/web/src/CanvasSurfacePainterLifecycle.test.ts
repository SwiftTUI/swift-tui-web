...
const image = (id: string, payload = true): WebHostSurfaceImage => ({
  id, format: "png", bounds: [0, 0, 1, 1], visibleBounds: [0, 0, 1, 1],
  scalingMode: "stretch", ...(payload ? { dataBase64: "QQ==" } : {}),
});
const frame = (...images: WebHostSurfaceImage[]): WebHostSurfaceFrame => ({
  version: 2, width: 2, height: 1, styles: [null], rows: [[]], images,
});
...
function decoded(id: string, closed: string[], width = 1): CanvasImageSource {
  return { width, height: 1, close: () => closed.push(id) } as unknown as CanvasImageSource;
}
...
test("decoded image LRU obeys byte budget and evicted transmit-once images recover", async () => {
  ...
  const painter = new CanvasSurfacePainter({
    ...
    decodeImage: async (_payload, _format, id) => { decodes++; return decoded(id, closed); },
    onImagePayloadMiss: (ids) => { misses.push([...ids]); },
  });
  painter.attach(canvas, () => {});
  for (const id of ["a", "b"]) { painter.paint(metrics, frame(image(id))); await flush(); }
  painter.paint(metrics, frame(image("a", false)));
  painter.paint(metrics, frame(image("c"))); await flush();
  expect(closed).toEqual(["b"]);
  painter.paint(metrics, frame(image("b", false)));
  painter.paint(metrics, frame(image("b", false)));
  expect(misses).toEqual([["b"]]);
  painter.paint(metrics, frame(image("b")), undefined, ["b"]); await flush();
...
test("current images may exceed soft limits without an eviction or decode loop", async () => {
  ...
  let redraws = 0;
  const painter = new CanvasSurfacePainter({
    maxDecodedImageCacheEntries: 1, maxDecodedImageCacheBytes: 1,
    decodeImage: async (_payload, _format, id) => { decodes++; return decoded(id, closed, 100); },
  });
  painter.attach(canvas, () => { redraws++; });
  painter.paint(metrics, frame(image("a"), image("b"))); await flush();
...
test("entry limit bounds decoded objects whose dimensions are unavailable", async () => {
  ...
  const painter = new CanvasSurfacePainter({
    ...
    decodeImage: async (_payload, _format, id) => ({ close: () => closed.push(id) }) as unknown as CanvasImageSource,
  });
  painter.attach(canvas, () => {});
  for (let i = 0; i < 10; i++) { painter.paint(metrics, frame(image(String(i)))); await flush(); }
  painter.paint(metrics, frame());
...
test("disposal closes retained and late decoded images and suppresses redraw and recovery", async () => {
  ...
  const misses: string[][] = [];
  const painter = new CanvasSurfacePainter({
    decodeImage: async (_payload, _format, id) => id === "pending"
      ? new Promise((resolve) => { resolvePending = resolve; }) : decoded(id, closed),
    onImagePayloadMiss: (ids) => { misses.push([...ids]); },
  });
  painter.attach(canvas, () => { redraws++; });
  painter.paint(metrics, frame(image("retained"))); await flush();
  painter.paint(metrics, frame(image("pending")));
  painter.dispose(); painter.dispose();
  resolvePending(decoded("pending", closed)); await flush();
...
test("a decode completing after its image disappeared releases its result", async () => {
  ...
  let redraws = 0;
  const painter = new CanvasSurfacePainter({
    decodeImage: () => new Promise((resolve) => { resolvePending = resolve; }),
  });
  painter.attach(canvas, () => { redraws++; });
