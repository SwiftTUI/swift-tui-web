...
test("linkTargetAt resolves runs through the deduplicated target table", () => {
  expect(linkTargetAt(links, targets, { x: 0.2, y: 0.9 })).toBe(
    "https://a.example/docs",
  );
  expect(linkTargetAt(links, targets, { x: 1.9, y: 0 })).toBe(
    "https://a.example/docs",
  );
  expect(linkTargetAt(links, targets, { x: 3.5, y: 0.5 })).toBe(
    "https://b.example",
  );
  expect(linkTargetAt(links, targets, { x: 1, y: 2 })).toBe(
    "https://a.example/docs",
  );
});
