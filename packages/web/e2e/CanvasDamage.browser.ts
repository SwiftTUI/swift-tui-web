...
for (const scale of [1, 2]) {
  test.describe(`Canvas damage at device scale ${scale}`, () => {
    ...
    for (const overlapping of [false, true]) {
      test(`partial repaint matches full pixels with ${overlapping ? "overlapping" : "one"} translucent image`, async ({
        page,
      }) => {
        await page.goto("/health");
        await page.addScriptTag({ url: "/canvas-damage.js", type: "module" });
        await page.waitForFunction(
          () => typeof window.runCanvasDamageAudit === "function",
        );
        const result = await page.evaluate(
          (overlap) => window.runCanvasDamageAudit(overlap),
          overlapping,
        );
    ...
    for (const [label, previous, next] of [
      ...
      test(`italic glyph ${label} stays inside its span and matches a full repaint`, async ({
        page,
      }) => {
        await page.goto("/health");
        await page.addScriptTag({ url: "/canvas-damage.js", type: "module" });
        await page.waitForFunction(
          () => typeof window.runCanvasGlyphDamageAudit === "function",
        );
        const result = await page.evaluate(
...
test("STUI-320: qualify full-grid clipping cost and oversized-glyph pixel ownership", async ({
  page,
}) => {
  await page.goto("/health");
  await page.addScriptTag({ url: "/canvas-damage.js", type: "module" });
  await page.waitForFunction(
    () => typeof window.runCanvasClipQualification === "function",
  );
